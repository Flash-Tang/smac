...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_agent(self, agent_id, side='red'):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in enemy_group.items():
                ...
                if (
                    dist < self.radars[agent_id] and e_unit.health > 0 and not self.jammed_by_any_enemy(agent_id)
                ):  # visible and alive
            ...
            for e_id, e_unit in enemy_group.items():
                ...
                if (
                    ...
                    if self.obs_all_health:
                        ...
                        en_health[e_id] = e_unit.health / e_unit.health_max
                        ind += 1
                        if self.shield_bits_enemy > 0:
                            max_shield = self.unit_max_shield(e_unit)
                            enemy_feats[e_id, ind] = (
    ...
    def init_units(self):
        ...
        while True:
            ...
            # init radars & jammers
            self.agents_radars = [{'wave_band': 'l', 'freq_point': 0} for _ in range(self.n_agents)]
            self.agents_jammers = [{'wave_band': 'l', 'freq_point': 0} for _ in range(self.n_agents)]

            self.enemy_radars = [{'wave_band': 'l', 'freq_point': 0} for _ in range(self.n_agents)]
            self.enemy_jammers = [{'wave_band': 'l', 'freq_point': 0} for _ in range(self.n_agents)]
    ...
    def jammed_by_any_enemy(self, agent_id):
        agent_wave_band, agent_freq_point = self.agents_radars[agent_id]['wave_band'], self.agents_radars[agent_id]['fre_point']
        agent_unit = self.get_unit_by_id(agent_id)
        for e_id, jammer in enumerate(self.enemy_jammers):
            if agent_freq_point == jammer['fre_point'] and agent_wave_band == jammer['wave_band']:
                e_unit = self.get_unit_by_id(e_id, blue_side=True)
                distance = self.distance(agent_unit.pos.x, agent_unit.pos.y,
                                             e_unit.pos.x, e_unit.pos.y)
                if distance <= jammer_radius[agent_wave_band]:
                    return True
