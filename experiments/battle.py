...
def main():
    ...
    #     with g1.as_default():
    #         red_leaders = get_leader_actors(env, n_groups)
    #         U.initialize()
    #         U.load_state('/tmp/policy/0.0')
            ...
            if False:
                with sess1.as_default():
                    with g1.as_default():
                        ...
                        policy_vec_n = [leader.action(obs) for leader, obs in zip(red_leaders, obs_n)]
                        policy_int_n = [np.argmax(policy_vec) for policy_vec in policy_vec_n]
                        # get action
                        red_act = p2a(policy_int_n, env)

                with sess2.as_default():
                    with g2.as_default():
                        _, blue_obs = env.get_obs()
    ...
    print('lose rate: {}'.format(lose_cnt / n_episodes))
